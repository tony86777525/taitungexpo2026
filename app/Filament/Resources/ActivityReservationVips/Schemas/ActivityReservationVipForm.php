...
class ActivityReservationVipForm
{
    public static function configure(Schema $schema): Schema
    {
        ...
        return $schema
            ->components([
                Hidden::make('type')
                    ->default(ActivityReservationType::VIP->value),
                // 1. 父級下拉選單
                ...
                Select::make('activity_session_id')
                    ...
                    ->options(function (Get $get) use ($activitySessions, $activityReservationCounts) {
                        ...
                        return $activitySessions->mapWithKeys(function ($activitySession) use ($activityReservationCounts) {
                            $currentVipCount = $activityReservationCounts->first(function ($count) use ($activitySession) {
                                return $activitySession->id === $count->activity_session_id && $count->type === ActivityReservationType::VIP->value;
                            })?->count ?? 0;

                            if (!$activitySession->canBookVipGroup($currentVipCount)) {
                                return [];
                            }

                            return [$activitySession->id => "{$activitySession->display_date} - {$activitySession->display_time_range}(剩餘{$currentVipCount}/{$activitySession->vip_group_count})"];
                        });
                    })
                    ->required()
                    ->live()
