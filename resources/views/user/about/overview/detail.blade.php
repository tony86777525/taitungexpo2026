...
@section('content')
    <main class="main">
        <div class="m-subPage">
            ...
            @endif
            {{-- 相關單位 --}}
            @if(!empty($project->getProjectUnitTypes()))
                <div class="m-element m-subPage__department">
                    <div class="container">
                        @foreach($project->getProjectUnitTypes() as $projectUnitType)
                            <div class="department">
                                <div class="department__title"><span class="f-h6">{{ $projectUnitType->display_name }}</span></div>
                                <ul class="department__list">
                                    @foreach($projectUnitType->units as $unit)
                                        <li><a
                                                @if(!empty($unit->link))
                                                    href="{{ $unit->link }}"
                                                @endif
                                            ><div class="imgWrap"><img src="{{ $unit->display_image }}" alt="{{ $unit->display_name }}"></div></a></li>
                                    @endforeach
                                </ul>
                            </div>
                        @endforeach
