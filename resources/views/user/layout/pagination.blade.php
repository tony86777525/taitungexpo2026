@if ($paginator->hasPages())
    <ul class="pagination">
        ...
        @else
            <li class="pagination__item pagination__item--prev"><a href="{{ $paginator->previousPageUrl() }}" class="text">PREV</a></li>
        @endif
        ...
        @if($currentPage > ($side + 2))
            <li class="pagination__item pagination__item--num"><a href="{{ $paginator->url(1) }}" class="text">1</a></li>
            <li class="pagination__item pagination__item--ellipsis"><span class="text"></span></li>
        @else
            @for($i = 1; $i < min($currentPage - $side, $lastPage); $i++)
                <li class="pagination__item pagination__item--num"><a href="{{ $paginator->url($i) }}" class="text">{{ $i }}</a></li>
            @endfor
        @endif
        ...
        @if($currentPage < ($lastPage - $side - 1))
            <li class="pagination__item pagination__item--ellipsis"><span class="text"></span></li>
            <li class="pagination__item pagination__item--num"><a href="{{ $paginator->url($lastPage) }}" class="text">{{ $lastPage }}</a></li>
        @else
        ...
        @if ($paginator->hasMorePages())
            <li class="pagination__item pagination__item--next"><a href="{{ $paginator->nextPageUrl() }}" class="text">NEXT</a></li>
        @else
