        ...
        <div class="cardItem__text">
            ...
            <ul class="links">
                ...
                @if($activity->show_tour_info === true)
                    <li class="f-h5">
                        {{ __('event.cards.group-tour.text') }}<a href="{{ $activity->project->display_reservation_url_from_event }}" class="reserveLink" target="_blank"><span class="reserveLink__text">{{ __('event.cards.group-tour.link-text') }}</span></a>
                    </li>
                @endif
            </ul>
        </div>
